...
import { getBlogsCategory } from "@/utils/getBlogs";
          ...
          <div className="flex flex-row gap-2">
            {Categories &&
              Categories?.data?.map((category: CategoriesDataProps, index: number) => {
                return (
                  <Badge key={index} className="block text-md bg-black text-white hover:bg-white hover:text-black duration-300">
                    {" "}
                    {category.category || " "}
                  </Badge>
                );
...
async function BlogCard({
  ...
  return (
    <div>
      <ContentCard title={title} tags={tags} createdAt={createdAt} category={category} author={author?.data?.name || "author"} id={id} />
    </div>
  );
}
